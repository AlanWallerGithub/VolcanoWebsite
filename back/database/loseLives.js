...
export async function loseLives(username, password, lives){
...
await con.connect(async function(err) {
  ...
  let queryPromise = new Promise(function(myResolve, myReject) {
    ...
    con.query("UPDATE users SET lives="+lives+" WHERE name = '"+username+"' AND password = '"+password+"'", function (err, result, fields) {
      if (err) throw err;
      myResolve(result);
    });
